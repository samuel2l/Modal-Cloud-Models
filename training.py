...
def train_model(
    ...
    inference_endpoint = os.environ.get(
        "MODAL_INFERENCE_URL",
        "https://kyeremehs--vibetune-inference-fastapi-app.modal.run/inference"
    )
    ...
    try:
        ...
        if os.path.exists(models_dir):
            ...
            if len(existing_models) > 10:
                ...
                print(f"[Training] ✅ No cleanup needed. {len(existing_models)} models total (keeping all).")
            
            # List available models after cleanup
            final_models = sorted([
                d for d in os.listdir(models_dir) 
                if os.path.isdir(os.path.join(models_dir, d))
            ])
            print(f"[Training] 📦 Available models in volume ({len(final_models)} total):")
            for model_name in final_models:
                marker = " ⭐ MAIN" if model_name == trained_model_id else " 💾 BACKUP" if model_name == training_job_id else ""
                print(f"[Training]    - {model_name}{marker}")
    except Exception as e:
        print(f"[Training] ⚠️  Cleanup failed: {e}")
    
    print(f"[Training] 🎉 Training pipeline completed successfully!")
    
    # Always log manual-update parameters (even if callback succeeded, for reference)
    print(f"[Training] 📋 Manual Update Parameters (if callback failed):")
    print(f"[Training]    Endpoint: POST /api/training/manual-update")
    print(f"[Training]    Request Body:")
    print(f"[Training]      {{")
    ...
    print(f"[Training]      }}")
    print(f"[Training]    Alternative (if main model deleted, use backup):")
    print(f"[Training]      \"trainedModelId\": \"{training_job_id}\"")
    print(f"[Training]    After updating, switch model at:")
    print(f"[Training]      {model_endpoint}/models/switch/{trained_model_id}")
    print(f"[Training]      (or use backup: {model_endpoint}/models/switch/{training_job_id})")
    return {
        "trainedModelId": trained_model_id,
        "status": "completed",
